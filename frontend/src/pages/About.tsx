		...
		<div className="flex flex-col min-h-screen bg-gray-900 text-white py-5 px-5">
			<div className="grow flex flex-nowrap items-center gap-5 justify-center py-4 flex-row">
				...
				<div className="h-full basis-1 grow">
					...
					<p className="max-w-xl text-lg">
						Welcome to Finduo, an online tool created with the purpose of
						helping the gaming community connect with each other based off of
						shared interests and goals. Find a new friend from across the world,
						or from a few cities over for when you're dedicated to hopping on
						the same server. Our goal is to streamline the process of finding
						friends to play your favorite games with. Through Finduo, building
						lasting connections with other like-minded gamers has never been
						easier. Hone your gameplay skills with your ideal partner by using
						our advanced search filters, reach out to a potential new friend by
						sending a party code through our messaging system, and don't forget
						to check your own inbox for invitations waiting on a response!
					</p>
				</div>
			</div>
			...
			<div className="mt-4 mx-8 flex-grow max-h-[50vh] flex overflow-x-hidden">
				<div className="mt-4 mx-8 flex animate-marquee">
					{userData.map((user: ProfileData, index) => (
						<div
							key={index}
							className="min-w-[25vw] relative group p-5 mx-8 rounded-xl overflow-hidden border-2 border-slate-600 hover:border-slate-300"
						>
							...
							<div className="w-max overflow-x-visible">
								<div className="flex flex-row">
									...
									<div>
										...
										<p className="text-sm">
											<span className="font-bold">Region:</span> {user.region}
										</p>
										<p className="text-sm">
										...
										<p className="text-sm">
											<span className="font-bold">Stars:</span> {user.stars}
										</p>
									</div>
								</div>
							</div>
				...
				<div className="flex animate-marquee2 whitespace-nowrap">
					{userData.map((user: ProfileData, index) => (
						<div
							key={index + userData.length}
							className="min-w-[25vw] relative group p-5 rounded-xl overflow-hidden border-2 border-slate-600 hover:border-slate-300"
						>
							<div className="absolute inset-0 transition-opacity duration-300 ease-out bg-slate-600 opacity-80 group-hover:opacity-40"></div>
							<div className="w-max overflow-x-visible">
								<div className="flex flex-row">
			...
			<div className="mt-4 flex-grow max-h-[50vh] flex overflow-hidden marquee-games">
				<div className="mt-4 flex animate-marquee-games">
					{gameNames.map((gameTitle: string, gameIndex) => (
						<div
							key={gameIndex}
							className="min-w-[10vw] relative group p-5 m-0 rounded-xl overflow-hidden border-2 border-slate-600 hover:border-slate-300 flex justify-center items-center"
						>
							<div className="absolute inset-0 transition-opacity duration-300 ease-out bg-slate-600 opacity-80 group-hover:opacity-40"></div>
				...
				<div className="mt-4 flex animate-marquee-games">
					{gameNames.map((gameTitle: string, gameIndex) => (
						<div
						key={gameIndex + gameNames.length}
						className="min-w-[10vw] relative group p-5 m-0 rounded-xl overflow-hidden border-2 border-slate-600 hover:border-slate-300 flex justify-center items-center"
					>
						<div className="absolute inset-0 transition-opacity duration-300 ease-out bg-slate-600 opacity-80 group-hover:opacity-40"></div>
						<div className="w-max overflow-hidden">
